...
def fit_neurons(p_idx, paths, n_idx, path, paths_list):
    ...
    output_path = path.replace('neuron_data', 'model_op')
    if not os.path.exists(output_path):
        ...
        for mod in model_combos:
            ...
            neuron_significant.append(armax_model.significant)
            neuron_rsqs.append(armax_model.r_sq)
        
        neuron_results = dict()
        neuron_results['params'] = neuron_params
        neuron_results['sig'] = neuron_significant
    ...
    for p_idx, paths in enumerate(paths_list):
        ...
        for n_idx, path in enumerate(paths):
            try:
                ...
                print("Fitting failed: exception")